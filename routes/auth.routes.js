...
router.post("/login", async (req, res) => {
    try {
        ...
        if (doctor) return res.status(200).json({ message: "login success",user_type:"doctor",email:doctor.email,username:doctor.doctor_id, fullName:doctor.fullName, doctor_id:doctor.doctor_id })

        const caretaker = await user_caretaker.findOne({ email, password });
        if (caretaker) return res.status(200).json({ message: "login success",user_type:"caretaker",email:caretaker.email,username:caretaker.caretaker_id, fullName:caretaker.fullName, caretaker_id:caretaker.caretaker_id })
